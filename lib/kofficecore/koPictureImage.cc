...
void KoPictureImage::scaleAndCreatePixmap(const QSize& size, bool fastMode)
{
    ...
    if ( fastMode )
    {
        img = m_originalImage.scale( size );
        m_cacheIsInFastMode=true;
    }
    else
    {
        img = m_originalImage.smoothScale( size );
        m_cacheIsInFastMode=false;
    }
    // Work around Qt 3.x Bug by using QPixmap::Color instead of Auto
    m_cachedPixmap.convertFromImage(img, QPixmap::Color);
    m_cachedSize=size;
}
