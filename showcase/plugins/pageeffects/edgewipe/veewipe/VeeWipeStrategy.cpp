...
VeeWipeStrategy::VeeWipeStrategy(int subtype, const char *smilSubType, bool reverse)
    : SCPageEffectStrategy(subtype, "veeWipe", smilSubType, reverse)
{
}
...
void VeeWipeStrategy::setup(const SCPageEffect::Data &data, QTimeLine &timeLine)
{
    Q_UNUSED(data);
    timeLine.setFrameRange(0, StepCount);
}

void VeeWipeStrategy::paintStep(QPainter &p, int currPos, const SCPageEffect::Data &data)
{
...
void VeeWipeStrategy::next(const SCPageEffect::Data &data)
{
