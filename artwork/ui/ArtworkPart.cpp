...
public:
    ...
    void applyCanvasConfiguration(ArtworkCanvas * canvas, ArtworkPart * part)
    {
    ...
    ArtworkDocument document;  ///< store non-visual doc info
...
ArtworkPart::ArtworkPart(QWidget* parentWidget, const char* widgetName, QObject* parent, const char* name, bool singleViewMode)
        : KoDocument(parentWidget, parent, singleViewMode), d(new Private())
{
    ...
    setObjectName(name);
    setComponentData(ArtworkFactory::componentData(), false);
    setTemplateType("artwork_template");
...
void ArtworkPart::setPageLayout(const KOdfPageLayoutData &layout)
{
    KoDocument::setPageLayout(layout);
    setPageSize(QSizeF(layout.width, layout.height));
}

KoView* ArtworkPart::createViewInstance(QWidget *parent)
{
...
void ArtworkPart::removeView(KoView *view)
{
    kDebug(38000) << "ArtworkPart::removeView";
    KoDocument::removeView(view);
}

void ArtworkPart::openTemplate(const KUrl &url)
{
...
bool ArtworkPart::loadXML(const KXmlDocument&, KOdfStore*)
{
    return false;
}

bool ArtworkPart::loadOdf(KOdfStoreReader &odfStore)
{
    ...
    KShapeLoadingContext shapeContext(context, resourceManager());
...
bool ArtworkPart::completeLoading(KOdfStore *store)
{
    bool ok = true;
    foreach(KDataCenterBase *dataCenter, dataCenterMap()) {
        ok = ok && dataCenter->completeLoading(store);
    }
    return ok;
}

void ArtworkPart::loadOasisSettings(const KXmlDocument &settingsDoc)
{
...
void ArtworkPart::saveOasisSettings(KOdfStore *store)
{
...
bool ArtworkPart::saveOdf(SavingContext &documentContext)
{
...
void ArtworkPart::slotDocumentRestored()
{
    setModified(false);
}

void ArtworkPart::paintContent(QPainter &painter, const QRect& rect)
{
...
ArtworkDocument& ArtworkPart::document()
{
    return d->document;
}

bool ArtworkPart::showStatusBar() const
{
    return d->showStatusBar;
}

void ArtworkPart::setShowStatusBar(bool b)
{
    d->showStatusBar = b;
}

uint ArtworkPart::maxRecentFiles() const
{
    return d->maxRecentFiles;
}

void ArtworkPart::reorganizeGUI()
{
    foreach(KoView* view, views()) {
        ArtworkView * kv = qobject_cast<ArtworkView*>(view);
        if (kv) {
            kv->reorganizeGUI();
...
void ArtworkPart::initConfig()
{
    KSharedConfigPtr config = ArtworkPart::componentData().config();
    ...
    setUnit(KUnit::unit(defaultUnit));
...
bool ArtworkPart::mergeNativeFormat(const QString &file)
{
    d->merge = true;
...
void ArtworkPart::addShape(KShape* shape)
{
...
void ArtworkPart::removeShape(KShape* shape)
{
    KShapeLayer *layer = dynamic_cast<KShapeLayer*>(shape);
    if (layer) {
...
QMap<QString, KDataCenterBase*> ArtworkPart::dataCenterMap() const
{
    return d->document.dataCenterMap();
}

void ArtworkPart::setPageSize(const QSizeF &pageSize)
{
