...
class ArtworkPathRefineCommand::Private {
public:
...
ArtworkPathRefineCommand::ArtworkPathRefineCommand(KPathShape * path, uint insertPointsCount, QUndoCommand *parent)
        : QUndoCommand(parent), d(new Private(path, insertPointsCount))
{
    setText(i18n("Refine path"));
}

ArtworkPathRefineCommand::~ArtworkPathRefineCommand()
{
    delete d;
}

void ArtworkPathRefineCommand::redo()
{
...
void ArtworkPathRefineCommand::undo()
{
    QUndoCommand::undo();
    d->path->update();
}
