...
#include "ArtworkWhirlPinchCommand.h"
...
class ArtworkWhirlPinchCommand::Private
{
public:
...
ArtworkWhirlPinchCommand::ArtworkWhirlPinchCommand(KPathShape * path, qreal angle, qreal pinch, qreal radius, QUndoCommand *parent)
        : QUndoCommand(parent), d(new Private(path, angle, pinch, radius))
{
    ...
    for (uint subpathIndex = 0; subpathIndex < subpathCount; ++subpathIndex) {
        ...
        for (int pointIndex = 0; pointIndex < pointCount; ++pointIndex) {
            KPathPoint * p = d->pathShape->pointByIndex(KoPathPointIndex(subpathIndex, pointIndex));
...
ArtworkWhirlPinchCommand::~ArtworkWhirlPinchCommand()
{
    delete d;
}

void ArtworkWhirlPinchCommand::redo()
{
    d->pathShape->update();
...
void ArtworkWhirlPinchCommand::undo()
{
    d->pathShape->update();
    uint subpathCount = d->pathData.count();
    for (uint subpathIndex = 0; subpathIndex < subpathCount; ++subpathIndex) {
