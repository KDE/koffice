...
#include "ArtworkCalligraphyTool.h"
#include "ArtworkCalligraphicShape.h"
#include "ArtworkCalligraphyOptionWidget.h"
...
ArtworkCalligraphyTool::ArtworkCalligraphyTool(KCanvasBase *canvas)
        : KToolBase(canvas), m_shape(0), m_angle(0),
...
ArtworkCalligraphyTool::~ArtworkCalligraphyTool()
{
}

void ArtworkCalligraphyTool::paint(QPainter &painter,
...
void ArtworkCalligraphyTool::mousePressEvent(KPointerEvent *event)
{
    if (m_isDrawing)
        return;
    ...
    m_isDrawing = true;
    m_pointCount = 0;
    m_shape = new ArtworkCalligraphicShape(m_caps);
    m_shape->setBackground(new KColorBackground(canvas()->resourceManager()->backgroundColor()));
    //addPoint( event );
}

void ArtworkCalligraphyTool::mouseMoveEvent(KPointerEvent *event)
{
    if (! m_isDrawing)
        return;
...
void ArtworkCalligraphyTool::mouseReleaseEvent(KPointerEvent *event)
{
    if (! m_isDrawing)
        return;
...
void ArtworkCalligraphyTool::addPoint(KPointerEvent *event)
{
    if (m_pointCount == 0) {
...
void ArtworkCalligraphyTool::setAngle(KPointerEvent *event)
{
    if (! m_useAngle) {
...
QPointF ArtworkCalligraphyTool::calculateNewPoint(const QPointF &mousePos,
        QPointF *speed)
{
...
qreal ArtworkCalligraphyTool::calculateWidth(qreal pressure)
{
...
qreal ArtworkCalligraphyTool::calculateAngle(const QPointF &oldSpeed,
...
void ArtworkCalligraphyTool::activate(ToolActivation, const QSet<KShape*> &)
{
...
QWidget *ArtworkCalligraphyTool::createOptionWidget()
{
    // if the widget don't exists yet create it
    ArtworkCalligraphyOptionWidget *widget = new ArtworkCalligraphyOptionWidget;
...
void ArtworkCalligraphyTool::setStrokeWidth(double width)
{
    m_strokeWidth = width;
}

void ArtworkCalligraphyTool::setThinning(double thinning)
{
    m_thinning = thinning;
}

void ArtworkCalligraphyTool::setAngle(int angle)
{
    m_customAngle = angle;
}

void ArtworkCalligraphyTool::setFixation(double fixation)
{
    m_fixation = fixation;
}

void ArtworkCalligraphyTool::setMass(double mass)
{
    m_mass = mass * mass + 1;
}

void ArtworkCalligraphyTool::setDrag(double drag)
{
    m_drag = drag;
}

void ArtworkCalligraphyTool::setUsePath(bool usePath)
{
    m_usePath = usePath;
    //if ( m_selectedPath )
...
void ArtworkCalligraphyTool::setUsePressure(bool usePressure)
{
    m_usePressure = usePressure;
}

void ArtworkCalligraphyTool::setUseAngle(bool useAngle)
{
    m_useAngle = useAngle;
}

void ArtworkCalligraphyTool::setCaps(double caps)
{
    m_caps = caps;
}

void ArtworkCalligraphyTool::updateSelectedPath()
{
