...
ArtworkCalligraphicShape::ArtworkCalligraphicShape(qreal caps)
        : m_lastWasFlip(false),
        m_caps(caps)
{
...
ArtworkCalligraphicShape::~ArtworkCalligraphicShape()
{
}

void ArtworkCalligraphicShape::appendPoint(const QPointF &point,
        qreal angle,
        qreal width)
{
    // convert the point from canvas to shape coordinates
    QPointF p = point - position();
    ArtworkCalligraphicPoint *calligraphicPoint =
        new ArtworkCalligraphicPoint(p, angle, width);
...
void ArtworkCalligraphicShape::
appendPointToPath(const ArtworkCalligraphicPoint &p)
{
...
void ArtworkCalligraphicShape::appendPointsToPathAux(const QPointF &p1,
        const QPointF &p2)
{
...
void ArtworkCalligraphicShape::smoothLastPoints()
{
    int index = pointCount() / 2;
    smoothPoint(index - 2);
    smoothPoint(index + 1);
...
void ArtworkCalligraphicShape::smoothPoint(const int index)
{
...
const QRectF ArtworkCalligraphicShape::lastPieceBoundingRect()
{
    if (pointCount() < 6)
        return QRectF();
...
bool ArtworkCalligraphicShape::flipDetected(const QPointF &p1, const QPointF &p2)
{
...
int ArtworkCalligraphicShape::ccw(const QPointF &p1,
                                 const QPointF &p2,
                                 const QPointF &p3)
{
...
void ArtworkCalligraphicShape::setSize(const QSizeF &newSize)
{
    QSizeF oldSize = size();
    // TODO: check
...
QPointF ArtworkCalligraphicShape::normalize()
{
...
void ArtworkCalligraphicShape::moveHandleAction(int handleId,
        const QPointF & point,
...
void ArtworkCalligraphicShape::updatePath(const QSizeF &size)
{
    Q_UNUSED(size);
    ...
    foreach(ArtworkCalligraphicPoint *p, m_points)
    appendPointToPath(*p);
...
void ArtworkCalligraphicShape::simplifyPath()
{
    if (m_points.count() < 2)
        return;
    ...
    artworkSimplifyPath(this, 0.3);
}

void ArtworkCalligraphicShape::addCap(int index1, int index2, int pointIndex,
                                     bool inverted)
{
...
QString ArtworkCalligraphicShape::pathShapeId() const
{
    return ArtworkCalligraphicShapeId;
}

void ArtworkCalligraphicShape::simplifyGuidePath()
{
    ...
    QList<ArtworkCalligraphicPoint *>::iterator i = m_points.begin() + 2;
