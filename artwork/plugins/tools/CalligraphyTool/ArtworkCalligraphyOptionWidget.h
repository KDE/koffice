...
#ifndef ARTWORKCALLIGRAPHYOPTIONWIDGET_H
#define ARTWORKCALLIGRAPHYOPTIONWIDGET_H
...
class ArtworkCalligraphyOptionWidget : public QWidget
{
    Q_OBJECT
public:
    ArtworkCalligraphyOptionWidget();
    ~ArtworkCalligraphyOptionWidget();

    // emits all signals with the appropriate values
    // called once the signals are connected inside ArtworkCalligraphyTool
    // to make sure all parameters are uptodate
    void emitAll();

signals:
    // all the following signals emit user friendly values, not the internal
    // values which are instead computed directly by ArtworkCalligraphyTool
...
#endif // ARTWORKCALLIGRAPHYOPTIONWIDGET_H
