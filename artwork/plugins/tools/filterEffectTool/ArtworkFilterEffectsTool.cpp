...
#include "ArtworkFilterEffectsTool.h"
...
class ArtworkFilterEffectsTool::Private
{
public:
    Private()
    ...
    void fillConfigSelector(KShape *shape, ArtworkFilterEffectsTool * tool)
    {
        if (!configSelector)
            return;
    ...
    void addWidgetForEffect(KFilterEffect * filterEffect, ArtworkFilterEffectsTool * tool)
    {
    ...
    EditMode editModeFromMousePosition(const QPointF &mousePosition, ArtworkFilterEffectsTool * tool)
    {
...
ArtworkFilterEffectsTool::~ArtworkFilterEffectsTool()
{
    delete d;
}

void ArtworkFilterEffectsTool::paint(QPainter &painter, const KViewConverter &converter)
{
...
void ArtworkFilterEffectsTool::repaintDecorations()
{
    if (d->currentShape && d->currentShape->filterEffectStack()) {
...
void ArtworkFilterEffectsTool::activate(ToolActivation toolActivation, const QSet<KShape*> &shapes)
{
...
void ArtworkFilterEffectsTool::mouseMoveEvent(KPointerEvent *event)
{
    if (currentStrategy()) {
...
KInteractionStrategy *ArtworkFilterEffectsTool::createStrategy(KPointerEvent *event)
{
    EditMode mode = d->editModeFromMousePosition(event->point, this);
    if (mode == None)
        return 0;
...
void ArtworkFilterEffectsTool::presetSelected(KoResource *resource)
{
    if (!d->currentShape)
        return;
...
void ArtworkFilterEffectsTool::editFilter()
{
...
void ArtworkFilterEffectsTool::filterChanged()
{
    if (! d->currentShape)
        return;
...
void ArtworkFilterEffectsTool::filterSelected(int index)
{
    if (! d->currentShape || ! d->currentShape->filterEffectStack())
        return;
...
void ArtworkFilterEffectsTool::regionXChanged(double x)
{
    if (!d->currentEffect)
        return;
...
void ArtworkFilterEffectsTool::regionYChanged(double y)
{
    if (!d->currentEffect)
        return;
...
void ArtworkFilterEffectsTool::regionWidthChanged(double width)
{
    if (!d->currentEffect)
        return;
...
void ArtworkFilterEffectsTool::regionHeightChanged(double height)
{
    if (!d->currentEffect)
        return;
...
QMap<QString, QWidget *> ArtworkFilterEffectsTool::createOptionWidgets()
{
...
#include "ArtworkFilterEffectsTool.moc"
