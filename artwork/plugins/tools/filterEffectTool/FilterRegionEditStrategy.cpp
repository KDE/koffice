...
FilterRegionEditStrategy::FilterRegionEditStrategy(KToolBase* parent, KShape * shape, KFilterEffect *effect, ArtworkFilterEffectsTool::EditMode mode)
...
void FilterRegionEditStrategy::handleMouseMove(const QPointF &mouseLocation, Qt::KeyboardModifiers modifiers)
{
    ...
    switch(m_editMode) {
        case ArtworkFilterEffectsTool::MoveAll:
            m_filterRect.translate(delta.x(), delta.y());
            break;
        case ArtworkFilterEffectsTool::MoveLeft:
            m_filterRect.setLeft(m_filterRect.left()+delta.x());
            break;
        case ArtworkFilterEffectsTool::MoveRight:
            m_filterRect.setRight(m_filterRect.right()+delta.x());
            break;
        case ArtworkFilterEffectsTool::MoveTop:
            m_filterRect.setTop(m_filterRect.top()+delta.y());
            break;
        case ArtworkFilterEffectsTool::MoveBottom:
