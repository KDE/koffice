...
# by converting .artwork -> .odg -> .artwork and comparing the initial and final .artwork files.
# We use the artwork format as a "dump" of the Artwork data, to check if everything is correct
# in memory, but the point is of course to ensure that the .odg has all the information.

# To use this script, you need to pass the full path to an existing artwork file as argument.
