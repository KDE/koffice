...
{
    return KCells::encodeColumnLabelText(col) + QString::number(row);
}
    ...
    for(int r = 0; r <= rowCount; ++r) {
        ...
        if (Row* row = m_context->sheet->row(r, false)) {
		    ...
		    typedef QPair<QString, QString> StringPair;
                    foreach( const StringPair &oleObject, cell->oleObjects ) {
...
KoFilter::ConversionStatus XlsxXmlWorksheetReader::read_c()
{
    ...
    TRY_READ_ATTR_WITHOUT_NS(r)
    if (!r.isEmpty()) {
        m_currentColumn = KCells::decodeColumnLabelText(r) - 1;
...
KoFilter::ConversionStatus XlsxXmlWorksheetReader::read_f()
{
    ...
    if (!ref.isEmpty()) {
        ...
        if (pos > 0) {
            ...
            const int c1 = KCells::decodeColumnLabelText(fromCell) - 1;
            const int r1 = KCells::decodeRowLabelText(fromCell) - 1;
            const int c2 = KCells::decodeColumnLabelText(toCell) - 1;
            const int r2 = KCells::decodeRowLabelText(toCell) - 1;
...
KoFilter::ConversionStatus XlsxXmlWorksheetReader::read_mergeCell()
{
    ...
    if (refList.count() >= 2) {
        ...
        if(rx.exactMatch(fromCell)) {
            ...
            const int fromCol = KCells::decodeColumnLabelText(fromCell) - 1;
...
KoFilter::ConversionStatus XlsxXmlWorksheetReader::read_hyperlink()
{
    ...
    if (!ref.isEmpty() && (!r_id.isEmpty() || !location.isEmpty())) {
        const int col = KCells::decodeColumnLabelText(ref) - 1;
        const int row = KCells::decodeRowLabelText(ref) - 1;
        if(col >= 0 && row >= 0) {
