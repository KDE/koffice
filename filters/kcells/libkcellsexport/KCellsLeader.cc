...
Leader::Leader(KoFilterChain *filterChain, KCellsBaseWorker *newWorker)
{
    m_worker = newWorker;
    m_filterChain = filterChain;
}
...
KCellsBaseWorker *Leader::getWorker() const
{
    return m_worker;
}


void Leader::setWorker(KCellsBaseWorker *newWorker)
{
    m_worker = newWorker;
}
...
KoFilter::ConversionStatus Leader::convert()
{
    ...
    if (!m_worker) {
        kWarning(30508) << "the KCellsWorker is NULL!";
        return KoFilter::StupidError;
    }

    // Gather data about the filter itself
    KCellsFilterProperty docProperty;
    ...
    KCellsDoc *document = (KCellsDoc *) m_filterChain->inputDocument();
    if (!document) {
        kWarning(30508) << "the KCellsDoc is NULL!";
        return KoFilter::StupidError;
    }
    if (!::qt_cast<const KCells::KCDoc *>(document)) {
        kWarning(30508) << "the document is not a KCellsDoc!";
        return KoFilter::StupidError;
    }
    if (document->mimeType() != "application/x-kcells") {
        kWarning(30508) << "the mime type document is not application/x-kcells!";
        return KoFilter::StupidError;
    }
    ...
    KCellsSheet *spreadSheet = document->map()->firstTable();
    while (spreadSheet != 0) {
        ...
        for (int row = 1; row <= m_maxCellRow; ++row) {
            for (int column = 1; column <= m_maxCellColumn; ++column) {
                KCCell spreadCell(spreadSheet, column, row);
...
KoFilter::ConversionStatus Leader::doInfo(KoDocumentInfo *info)
{
    ...
    KCellsFilterProperty docInfoLogProperty;
    ...
    KCellsFilterProperty docInfoAuthorProperty;
    ...
    KCellsFilterProperty docInfoAboutProperty;
...
KoFilter::ConversionStatus Leader::doSpreadBook(KCellsDoc *document)
{
    KCellsFilterProperty docSpreadBookProperty;
...
KoFilter::ConversionStatus Leader::doSpreadSheet(KCellsSheet *spreadSheet)
{
    KCellsFilterProperty docSpreadSheetProperty;
...
KoFilter::ConversionStatus Leader::doSpreadCell(const KCCell& spreadCell, int column, int row)
{
    KCellsFilterProperty docSpreadCellProperty;
...
void Leader::updateMaxCells(KCellsSheet *spreadSheet)
{
    m_maxCellColumn = 0;
    m_maxCellRow = 0;
