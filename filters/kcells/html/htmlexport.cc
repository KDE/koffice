...
{
    if (to != "text/html" || from != "application/x-kcells") {
...
void HTMLExport::openPage(KCSheet *sheet, KoDocument *document, QString &str)
{
    ...
    str += "content=\"KCells HTML Export Filter Version = ";
...
void HTMLExport::convertSheet(KCSheet *sheet, QString &str, int iMaxUsedRow, int iMaxUsedColumn)
{
    ...
    // Either we get hold of KCellsTable::m_dctCells and apply the old method below (for sorting)
    // or, cleaner and already sorted, we use KCellsTable's API (slower probably, though)
    ...
    for (int currentrow = 1 ; currentrow <= iMaxUsedRow ; ++currentrow, ++i) {
        ...
        for (int currentcolumn = 1 ; currentcolumn <= iMaxUsedColumn ; currentcolumn++) {
            ...
            // "208.00" in KCells will be "208" in HTML (not always?!)
            bool link = false;

            if (!cell.link().isEmpty()) {
                if (KCells::localReferenceAnchor(cell.link())) {
                    text = cell.userInput();
                } else {
