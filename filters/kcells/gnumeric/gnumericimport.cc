...
#include <kcells/KCLoadingInfo.h>
...
//Sheet2!$A$2:$D$8
    ...
    if (pos != -1) {
        ...
        if (pos != -1) {
            ...
            rect.setLeft(KCells::decodeColumnLabelText(left.left(pos)));
            rect.setTop(left.right(left.length() - pos - 1).toInt());

            pos = right.indexOf('$');
            rect.setRight(KCells::decodeColumnLabelText(right.left(pos)));
            rect.setBottom(right.right(right.length() - pos - 1).toInt());
        } else {
            QString left = _zone;
            int pos = left.indexOf('$');
            int leftPos = KCells::decodeColumnLabelText(left.left(pos));
...
void setSelectionInfo(QDomNode * sheet, KCSheet * /* table */)
{
    ...
    while (!selection.isNull()) {
        ...
        QRect kcells_selection;

        kcells_selection.setLeft((e.attribute("startCol").toInt() + 1));
        kcells_selection.setTop((e.attribute("startRow").toInt() + 1));
        kcells_selection.setRight((e.attribute("endCol").toInt() + 1));
        kcells_selection.setBottom((e.attribute("endRow").toInt() + 1));

        /* can't set it in the table -- must set it to a view */
        //   table->setSelection(kcells_selection);
...
void convertToPen(QPen & pen, int style)
{
    ...
    case 6:
        ...
        break;
    case 7:
        // very thin dots => no match in KCells
        pen.setStyle(Qt::DotLine);
        pen.setWidth(1);
        break;
    case 8:
...
void GNUMERICFilter::ParseBorder(QDomElement & gmr_styleborder, const KCCell& kcells_cell)
{
...
bool GNUMERICFilter::setType(const KCCell& kcells_cell,
    ...
    for (i = 0; cell_date_format[i] ; ++i) {
        ...
        if ((formatString == "d/m/yy") || (formatString == cell_date_format[i])) {
            ...
            if (!kcells_cell.isDate()) {
                ...
                date = kcells_cell.value().asDate(kcells_cell.sheet()->map()->calculationSettings());
            ...
            KCCell cell(kcells_cell);
            cell.setValue(KCValue(date, kcells_cell.sheet()->map()->calculationSettings()));
            KCStyle style;
    ...
    for (i = 0; cell_time_format[i] ; ++i) {
        if (formatString == cell_time_format[i]) {
            ...
            if (!kcells_cell.isTime()) {
                ...
                time = kcells_cell.value().asTime(kcells_cell.sheet()->map()->calculationSettings());
            ...
            KCCell cell(kcells_cell);
            cell.setValue(KCValue(time, kcells_cell.sheet()->map()->calculationSettings()));
            KCStyle style;
            style.setFormatType(type);
            cell.setStyle(style);
...
void GNUMERICFilter::ParseFormat(QString const & formatString, const KCCell& kcells_cell)
{
    ...
    if (l == 0 || formatString == "General") {
        ...
        if ((formatString[0] == '[') && (formatString[1] == '$')) {
            ...
            QString content(kcells_cell.value().asString());

            if (setType(kcells_cell, formatString, content))
                return;

            if (formatString.indexOf("?/?") != -1) {
                // TODO: fixme!
                style.setFormatType(KCFormat::fraction_three_digits);
                KCCell(kcells_cell).setStyle(style);
                return;
            }
    ...
    if (formatString[lastPos] == '#') {
        ...
        if (formatString[lastPos + 1] == ',')
            ...
            sep = false;
        // since KCells 1.3
        // kcells_cell.setThousandsSeparator( sep );
    }
        ...
        while (!style_region.isNull()) {
            ...
            for (column = startCol; column <= endCol; ++column) {
                for (row = startRow; row <= endRow; ++row) {
                    ...
                    KCCell kcells_cell(table, column, row);
                    KCStyle style;

                    // don't create new cells -> don't apply format on empty cells, if bigger region
                    if ((kcells_cell.isDefault() || kcells_cell.isEmpty())
                    ...
                    kDebug(30521) << "Style valid for kcells";
                    kcells_cell = KCCell(table, column, row);
                    ...
                    if (!validation.isNull()) {
                        ...
                        if (!validation_element.isNull()) {
                            ...
                            if (validation_element.hasAttribute("Type")) {
                                ...
                                switch (valueOp) {
                                case 0:
                                    kcells_validity.setRestriction(KCValidity::None);
                                    break;
                                case 1: {
                                    kcells_validity.setRestriction(KCValidity::Integer);
                                    if (validation_element.hasAttribute("Operator")) {
                                        ...
                                        switch (value) {
                                        case 0:
                                            kcells_validity.setCondition(KCConditional::Between);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            if (!expression1.isNull())
                                                kcells_validity.setMaximumValue(value2);
                                            break;
                                        case 1:
                                            kcells_validity.setCondition(KCConditional::DifferentTo);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            if (!expression1.isNull())
                                                kcells_validity.setMaximumValue(value2);
                                            break;
                                        case 2:
                                            kcells_validity.setCondition(KCConditional::Equal);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            break;
                                        case 3:
                                            kcells_validity.setCondition(KCConditional::Different);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            break;
                                        case 4:
                                            kcells_validity.setCondition(KCConditional::Superior);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            break;
                                        case 5:
                                            kcells_validity.setCondition(KCConditional::Inferior);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            break;
                                        case 6:
                                            kcells_validity.setCondition(KCConditional::SuperiorEqual);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            break;
                                        case 7:
                                            kcells_validity.setCondition(KCConditional::InferiorEqual);
                                            if (!expression0.isNull())
                                                kcells_validity.setMinimumValue(value1);
                                            break;
                                        default:
                                ...
                                break;
                                case 2:
                                    kcells_validity.setRestriction(KCValidity::KCNumber);
                                    if (validation_element.hasAttribute("Operator")) {
                                        const KCValue value1(expression0.toElement().text().toInt());
                                        const KCValue value2(expression1.toElement().text().toInt());
                                        int value = validation_element.attribute("Operator").toInt();
                                    ...
                                    break;
                                case 3:
                                    kcells_validity.setRestriction(KCValidity::List);
                                    break;
                                case 4:
                                    kcells_validity.setRestriction(KCValidity::Date);
                                    if (validation_element.hasAttribute("Operator")) {
                                    ...
                                    break;
                                case 5:
                                    kcells_validity.setRestriction(KCValidity::Time);
                                    if (validation_element.hasAttribute("Operator")) {
                                    ...
                                    break;
                                case 6:
                                    kcells_validity.setRestriction(KCValidity::TextLength);
                                    if (validation_element.hasAttribute("Operator")) {
                                        const KCValue value1(expression0.toElement().text().toInt());
                                        const KCValue value2(expression1.toElement().text().toInt());
                                        int value = validation_element.attribute("Operator").toInt();
                    ...
                    if (!font.isNull()) {
                        ...
                        if (!hyperlink.isNull()) {
                            ...
                            if (hyperlink.toElement().hasAttribute("type")) {
                                ...
                                if (!tip.isEmpty()) {
                                    kcells_cell.setUserInput(tip);
                                    kcells_cell.setValue(KCValue(tip));
                                }
                                if (linkType == "GnmHLinkURL") {
                                    if (!target.startsWith("http://"))
                                        target = "http://" + target;
                                    kcells_cell.setLink(target);
...
KoFilter::ConversionStatus GNUMERICFilter::convert(const QByteArray & from, const QByteArray & to)
{
    ...
    if (from != "application/x-gnumeric" || to != "application/x-kcells") {
            ...
            if (!e.isNull()) { // the node was really an element.
                ...
                if (!content_node.isNull()) {
                    ...
                    if (!content.isNull()) { // the node was really an element.
                        ...
                        KCCell kcells_cell = KCCell(table, column, row);
                        KCStyle style;
                        ...
                        if (e.hasAttribute("ValueType")) {
                            ...
                            if (valuetype == "40") { //percentage
                                style.setFormatType(KCFormat::Percentage);
                                kcells_cell.setValue(KCValue(cell_content));
                        ...
                        if (e.hasAttribute("ValueFormat")) {
                            ...
                            if (!setType(kcells_cell, formatString, cell_content))
