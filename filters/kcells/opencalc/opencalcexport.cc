...
#include <kcells/part/AboutData.h> // for version
...
#include <kcells/kcells_limits.h>
...
K_EXPORT_PLUGIN(OpenCalcExportFactory("kcellsopencalcexport", "kofficefilters"))
...
{
    /* later...
       KCellsLeader  * leader = new KCellsLeader( m_chain );
    ...
    */
    ...
    if ((to != "application/vnd.sun.xml.calc") || (from != "application/x-kcells")) {
...
void exportNamedExpr(KCDoc* kcellsDoc, QDomDocument & doc, QDomElement & parent,
                     AreaList const & namedAreas)
{
    ...
    for (int i = 0; i < namedAreas.count(); ++i) {
        ...
        sheet = kcellsDoc->map()->namedAreaManager()->sheet(namedAreas[i]);
        if (!sheet)
            continue;
        range = kcellsDoc->map()->namedAreaManager()->namedArea(namedAreas[i]).firstRange();
...
bool OpenCalcExport::exportBody(QDomDocument & doc, QDomElement & content, const KCDoc * ksdoc)
{
    ...
    KoDocument * document   = m_chain->inputDocument();
    KCDoc * kcellsDoc = static_cast<KCDoc *>(document);

    AreaList namedAreas = kcellsDoc->map()->namedAreaManager()->areaNames();
    if (namedAreas.count() > 0) {
        QDomElement namedExpr = doc.createElement("table:named-expressions");
        exportNamedExpr(kcellsDoc, doc, namedExpr, namedAreas);
                                 ...
                                 const KCSheet *sheet, int row, int maxCols)
{
    ...
    while (i <= maxCols) {
        ...
        if (cell.isFormula()) {
            ...
            if (KCells::localReferenceAnchor(tmp))
