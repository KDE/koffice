...
KoFilter::ConversionStatus SvgExport::convert(const QByteArray& from, const QByteArray& to)
{
    ...
    saveDocument(artworkPart->document());
...
void SvgExport::saveDocument(ArtworkDocument& document)
{
    ...
    *m_defs <<
    "<!-- Created using Artwork, part of koffice: http://www.koffice.org/artwork -->" << endl;
...
void SvgExport::saveGroup(KShapeContainer * group)
{
    printIndentation(m_body, m_indent++);
