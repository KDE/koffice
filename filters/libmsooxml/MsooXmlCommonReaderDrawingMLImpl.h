...
//! @todo for pptx: maybe use KOdfGenericStyle::PresentationAutoStyle?
    ...
    if (m_currentDrawingObject->m_positions.contains(XlsxDrawingObject::FromAnchor)) {  // if we got 'from' cell
        ...
        if (m_currentDrawingObject->m_positions[XlsxDrawingObject::ToAnchor].m_col >= 0) {
            body->addAttribute("table:end-cell-address", KCells::encodeColumnLabelText(m_currentDrawingObject->m_positions[XlsxDrawingObject::ToAnchor].m_col+1) +
...
void MSOOXML_CURRENT_CLASS::generateFrameSp()
{
    ...
    if (m_currentDrawingObject->m_positions.contains(XlsxDrawingObject::FromAnchor)) {
        ...
        if (m_currentDrawingObject->m_positions.contains(XlsxDrawingObject::ToAnchor)) {
            ...
            body->addAttribute("table:end-cell-address", KCells::encodeColumnLabelText(f.m_col+1) + QString::number(f.m_row+1));
...
//! @todo add ELSE_WRONG_FORMAT
    ...
    if (m_currentBulletProperties.bulletFont() == "Wingdings" && !m_currentBulletProperties.bulletChar().isEmpty()) {
