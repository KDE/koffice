...
# This script helps finding out problems in filter of kcells/kword/kpresenter
...
case "$1" in 
	...
	case "$3" in 
	    ...
	    ;;
	    *)
	    printf "Usage: %s {kcells|kword|kpresenter} <file name> {gnumeric|opencalc}\n" "$0"
	    exit 1;
	esac
	;;
	kword)
	...
	case "$3" in 
	    ...
	    ;;
	    *)
	    printf "Usage: %s {kcells|kword|kpresenter} <file name> {rtf|amipro|oowriter|latex|paldoc|abiword|wml|wordperfect|ascii}\n" "$0"
	    exit 1;
	esac
	;;
	krita)
	...
	case "$3" in
		...
		;;
                ...
                ;;
		*)
		printf "Usage: %s {kcells|kword|kpresenter|krita} <file name> {magick-png}\n" "$0"
		exit1;
	esac
	;;
	kpresenter)
	...
	case "$3" in 
	    ...
	    ;;
	    *)
	    printf "Usage: %s {kcells|kword|kpresenter} <file name> {ooimpress}\n" "$0"
	    exit 1;
	esac
	;;
	*)
	printf "Usage: %s {kcells|kword|kpresenter|krita} <file name> <type of filter>\n" "$0"
	exit 1;
esac
