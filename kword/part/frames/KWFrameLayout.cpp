...
static bool sortFramesByFS(const KWFrame *frame1, const KWFrame *frame2)
{
    Q_ASSERT(frame1);
    Q_ASSERT(frame1->frameSet());
    Q_ASSERT(frame2);
    Q_ASSERT(frame2->frameSet());
    if (frame1->frameSet() == frame2->frameSet()) {
        foreach (KWFrame *f, frame1->frameSet()->frames()) {
            if (f == frame1)
                return true;
            if (f == frame2)
                return false;
    ...
    return frame1->frameSet() < frame2->frameSet();
}
...
//kDebug() <<"createNewFramesForPage" << pageNumber;
    ...
    if (page.pageSide() == KWPage::PageSpread) {
        ...
        if (page.pageStyle().hasMainTextFrame()) {
            int columns = page.pageStyle().columns().columns;
            KWTextFrameSet *fs = getOrCreate(KWord::MainTextFrameSet, page);
            QRectF rect(QPointF(page.width() / 2, page.offsetInDocument()),
    ...
    bool odd = false; // look an odd number of pages back, so frameOnBothSheets is used
    for (int i = pageNumber - 2; i < pageNumber; i++) {
        ...
        qSort(frames.begin(), frames.end(), sortFramesByFS);

        KWFrameSet *lastFs = 0;
        foreach (KWFrame *frame, framesInPage(pageRect)) {
            if ((odd && !frame->frameOnBothSheets()) || (!odd && frame->frameOnBothSheets()))
                continue;
            if (!(frame->frameSet()->newFrameBehavior() == KWord::ReconnectNewFrame ||
                    frame->frameSet()->newFrameBehavior() == KWord::CopyNewFrame))
                continue;
           ...
           if (pageRect.bottom() < lastFs->frames().last()->shape()->position().y())
            ...
            if (tfs && lastFs->newFrameBehavior() != KWord::CopyNewFrame) {
                ...
                KWFrameSet *fs = lastFs;
                Q_ASSERT(fs);
                Q_ASSERT(fs->newFrameBehavior() == KWord::CopyNewFrame);
                Q_ASSERT(fs->frameCount() > 0);
                KWCopyShape *shape = new KWCopyShape(fs->frames().first()->shape(), m_pageManager);
                shape->setShapeSeriesPlacement(fs->shapeSeriesPlacement());
                f = new KWFrame(shape, fs);
            }
    ...
    if (columns > 0 && main[0]) {
        ...
        for (int i = columns - 1; i >= 0; i--) {
            ...
            shape->setSize(QSizeF(columnWidth -
                                  (first ? 0 : pageStyle.columns().columnSpacing),
