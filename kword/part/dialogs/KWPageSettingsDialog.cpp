...
void KWPageSettingsDialog::accept()
{
    ...
    bool onlyThisPage = !applyToDocument();
    if (onlyThisPage) {
        // check if we really have to detach styles to only affect one page.
        bool foundSecond = false;
        foreach (const KWPage &page, m_document->pageManager()->pages()) {
            if (page == m_page)
                continue;
            if (page.pageStyle() == styleToUpdate) {
                foundSecond = true;
                break;
            }
        }
        if (!foundSecond) {
            // if there is only one user of this page, so go ahead and apply to 'all' pages
            onlyThisPage = false;
        }
    }
    if (onlyThisPage) {
        // detach to create a style specifically for this page.
        int i = m_page.pageNumber();
        const QString baseName("AutogeneratedStyle%1");
        while (true) {
            QString newName(baseName.arg(i));
            if (!m_document->pageManager()->pageStyle(newName).isValid()) {
                styleToUpdate.detach(newName);
                break;
            }
            ++i;
        }
