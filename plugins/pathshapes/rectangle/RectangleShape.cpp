...
bool RectangleShape::loadOdf(const KoXmlElement &element, KoShapeLoadingContext &context)
{
    ...
    if (element.hasAttributeNS(KOdfXmlNS::svg, "rx") && element.hasAttributeNS(KOdfXmlNS::svg, "ry")) {
        qreal rx = KUnit::parseValue(element.attributeNS(KOdfXmlNS::svg, "rx", "0"));
        qreal ry = KUnit::parseValue(element.attributeNS(KOdfXmlNS::svg, "ry", "0"));
        m_cornerRadiusX = rx / (0.5 * size().width()) * 100;
        m_cornerRadiusY = ry / (0.5 * size().height()) * 100;
    } else {
        QString cornerRadius = element.attributeNS(KOdfXmlNS::draw, "corner-radius", "");
