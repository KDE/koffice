...
setFormatProperty(textBlockFormat, KParagraphStyle.TabPositions, [t2in]);
cursor.setBlockFormat(textBlockFormat);
cursor.insertText("some text\t(tab)\t(tab) position of tab stop is 2in.");
cursor.insertBlock(defaultBlockFormat);

setFormatProperty(textBlockFormat, KParagraphStyle.TabPositions, [t1in, t2in, t3dot5in, t5in]);
cursor.setBlockFormat(textBlockFormat);
cursor.insertText("some text\t(tab)\t(tab) \t(tab) position of tab stops are 1in, 2in, 3.5in, 5in.");
cursor.insertBlock(defaultBlockFormat);

setFormatProperty(textBlockFormat, KParagraphStyle.TabPositions, [t100pt]);
cursor.setBlockFormat(textBlockFormat);
